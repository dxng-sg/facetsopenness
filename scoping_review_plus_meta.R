...
# Neither Egger's regression test (p = 0.3071) or the Rank correlation test (p = 0.3918) was statistically significant so there's no evidence of publication bias 
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_values, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_aesthetics, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_feelings, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_fantasy, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_ideas, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_actions, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
# "tau^2 (estimated amount of total heterogeneity): 0.1045 (SE = 0.0794)"

# "I^2 (total heterogeneity / total variability):   95.65%"

# This line indicates that I^2 was 95.65%. In other words 95.65% of variation reflected actual differences in the population mean. 
# The confidence interval test revealed the 95% confidence interval for this value is 87.68, 99.44.

# "Test for Heterogeneity: 
# Q(df = 4) = 142.8767, p-val < 0.0001"
...
#
# estimate       se     zval     pval    ci.lb    ci.ub          
#   0.3990   0.1499   2.6622   0.0078   0.1052   0.6927      ** 
...
#pred    ci.lb  ci.ub  cr.lb  cr.ub
#0.379 0.105 0.600 -0.291 0.800

# These two lines display the transformation of Fisher's z back to Pearson's r ("pred"), and the 95% confidence interval for r ("ci.lb" and "ci.ub") for 
# reporting the meta-analysis.

#        estimate   ci.lb   ci.ub
#tau^2    0.1045  0.0338  0.8535
#tau      0.3233  0.1839  0.9239
#I^2(%)  95.6511 87.6851 99.4464
#H^2      22.9943  8.1203  180.6395
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_values_t, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_aesthetics_t, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
# This line indicates that I^2 was 70.94%. In other words 79.38% of variation reflected actual differences in the population mean. 
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_feelings_t, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
# This line indicates that I^2 was 73.47%. In other words 72.77% of variation reflected actual differences in the population mean. 
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_fantasy_t, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_ideas_t, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
...
mes_ss <-  robu(formula = yi ~ 1, data = dat_actions_t, studynum = study_id, var.eff.size = vi, modelweights = "HIER", small = TRUE)
